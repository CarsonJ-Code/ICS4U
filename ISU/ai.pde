...
int scoreProvinceForAIDecisonMakingInAsFarAsItIsDecidingToPickATargetToMoveATroop(int provinceID, int faction){
    ...
    if(hasATroopThatIsOwnedAndControlledByAnEnemyFactionAndCanBeUsedForPotentiallyHostileActionsAgainstTheFactionPassedIntoThisFunction(provinceID, faction)) score -= 3;

    return 1;
}

boolean hasATroopThatIsOwnedAndControlledByAnEnemyFactionAndCanBeUsedForPotentiallyHostileActionsAgainstTheFactionPassedIntoThisFunction(int provinceID, int factionID){}